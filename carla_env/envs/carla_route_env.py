    ...
    def close(self):
        try:
            ...
            if self.carla_process:
                ...
                self.carla_process.terminate()  # Terminate CARLA process
            
            pygame.quit()  # Quit pygame
            print("Pygame quit.")

            # Destroy all actors (vehicles, sensors, etc.)
            if self.world is not None:
                print("Destroying all actors...")
                vehicles = self.world.get_actors().filter('vehicle.*')
                for vehicle in vehicles:
                    vehicle.destroy()  # Destroy each vehicle

                # Destroy sensors
                sensors = self.world.get_actors().filter('sensor.*')
                for sensor in sensors:
                    sensor.destroy()  # Destroy each sensor

            self.closed = True  # Mark the environment as closed
            print("Closed environment successfully.")
        except Exception as e:
